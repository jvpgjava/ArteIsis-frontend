...
@Component({
  selector: 'app-home',
  standalone: true,
  imports: [Hero, FeaturedProducts, Button, Input, Select, Textarea, MatIconModule, FormsModule],
  template: `
    <app-hero />
    ...
    <section class="py-24 px-8 md:px-12 bg-white" id="contato">
      <div class="max-w-4xl mx-auto text-center">
        ...
        <form class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left bg-isis-light/30 p-10 md:p-16 rounded-[2.5rem] border border-isis-blue/5" (ngSubmit)="submitContact()">
          <app-input label="Nome" placeholder="Seu nome completo" [(value)]="contactName" />
          <app-input label="E-mail" type="email" placeholder="seu@email.com" [(value)]="contactEmail" />

          <app-select label="Assunto" class="md:col-span-2" [options]="contactSubjectOptions" [(value)]="contactSubject" />

          <app-textarea label="Sua Ideia" class="md:col-span-2" placeholder="Conte-nos um pouco sobre a estampa que você deseja..." [(value)]="contactMessage" />
    ...
    @if (contactThanksOpen()) {
      <div
        class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-isis-dark/45 backdrop-blur-[2px]"
        role="dialog"
        aria-modal="true"
        aria-labelledby="contact-thanks-message"
        (click)="closeContactThanks()"
      >
        <div
          class="relative w-full max-w-md rounded-[2rem] bg-white p-10 md:p-12 shadow-2xl shadow-isis-blue/15 border border-isis-blue/10 text-center"
